...
| Вкладка **Контрольная панель** — виджеты управления | ✅ Расширена |
...
| Фоновый reader — batch-чтение тегов каждые 500 мс | ✅ Новый |
| GET /api/tags и /api/io — возврат кэша мгновенно | ✅ Новый |
| Секции (заголовки-разделители) в Контрольной панели | ✅ Новый |
| Drag-and-drop сортировка виджетов | ✅ Новый |
| Кнопка + Индикатор в одной карточке | ✅ Новый |
...
- **Блок подключения** (вверху): IP, Slot, кнопка «Подключить/Отключить», история 10 последних подключений, алерты
...
- **localStorage**: запоминает IP/Slot и кэширует полный список тегов. После перезагрузки страницы теги видны без переподключения к ПЛК.
...
GET /api/tags ──────────►  вернуть _tags_cache (0 мс)
                            _plc_bg_reader() ──────────► plc.Read([все теги])
                                             ◄──────────  все значения за 1 вызов
                            обновить _tags_cache

POST /api/write ────────►  async with _plc_lock ──────► plc.Write(tag, value)
                           (ждёт ≤ время batch-чтения)  ◄─────── ок
```
...
- Каждые 500 мс вызывает `plc.Read([список_всех_тегов])` — **один batch-запрос** вместо N отдельных
...
- `POST /api/write` берёт `_plc_lock` и пишет в отдельном потоке через `asyncio.to_thread`; максимальная задержка = время одного batch-чтения (~50 мс)
...
### CSS кэш-бастинг
Все статические файлы подключены с `?v=15` в `<link>` и `<script>` тегах.
...
| Теги не показываются в Контрольной панели | `window.tagsData` не заполнен — нужно сначала подключиться к ПЛК на вкладке «ПЛК» |
